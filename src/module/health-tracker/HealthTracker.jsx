...
const HealthTracker = () => {
  ...
  const [inputType, setInputType] = useState("glucose");
  ...
  const todayStats = useMemo(() => {
    ...
    const sugar = todayEntries
      .filter((entry) => entry.type === "sugar" || entry.type === "glucose")
      .reduce((sum, entry) => sum + Number(entry.value), 0);
  ...
  const getFilterCounts = () => {
    const counts = {
      ...
      sugar: entries.filter((e) => e.type === "sugar" || e.type === "glucose").length,
